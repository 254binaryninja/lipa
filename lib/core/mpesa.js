...
class Mpesa {
    ...
    async initiateC2B(shortCode,confirmationURL,validationURL,validate) {
        const token = await this.generateAccessToken();
        const response = await c2B(this.apiUrl,shortCode,confirmationURL,validationURL,token,validate);
        return response;
    }
}